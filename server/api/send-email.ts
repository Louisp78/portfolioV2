...
export default defineEventHandler(async (event) => {
  ...
  const itemListHtml = body.itemList.map((item: QuoteItem) => `
  ...
  htmlContent = htmlContent
    ...
    .replace('{{projectDescription}}', body.projectDescription || 'Aucune description fournie.')
  ...
  const textContent = `
    ...
    ${body.itemList.map((item: QuoteItem, index: number) => `
      - Élément ${index + 1} :
  ...
  return sendMail({ subject: 'Nuxt + nodemailer', text: textContent, html: htmlContent, to: 'louis.place@epita.fr' })
})
